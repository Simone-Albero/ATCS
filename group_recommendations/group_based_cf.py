...
def main():
    ...
    recommended_items = [x[0] for x in recc.groupAveragePred(df, users, 4)]
    new_row = ['groupAveragePred']
    for user in users:
        new_row.append(recc.getSatisfaction(df, recommended_items, user))
    stats.loc[len(stats)] = new_row

    recommended_items = [x[0] for x in recc.groupLeastMiseryPred(df, users, 4)]
    new_row = ['groupLeastMiseryPred']    
    for user in users:
        new_row.append(recc.getSatisfaction(df, recommended_items, user))
    stats.loc[len(stats)] = new_row

    recommended_items = [x[0] for x in recc.customRecommendations(df, users, 4)]
    new_row = ['customRecommendations']    
    for user in users:
        new_row.append(recc.getSatisfaction(df, recommended_items, user))
    stats.loc[len(stats)] = new_row

    recommended_items = recc.sequentialRecommendations(df, users, 4)
